...
public:
  ...
  //Space Complexity O(n)
    string sortSentence(string s) {
        // #define ppq pair<int,string>
        typedef pair<int,string> ppq;
        priority_queue<ppq,vector<ppq>,greater<ppq>> pq;
        int start =0;
  ...
  //Space Complexity O(n)
        ...
        string k;    
        while(ss>>k){
                pq.push({k[k.size()-1]-'0',k.substr(0,k.size()-1)});  
